...
def create_root_ca(common_name, key_path, cert_path):
    ...
    certificate = x509.CertificateBuilder().subject_name(
        ...
        datetime.utcnow() + timedelta(days=31*365)
    ).add_extension(
...
def sign_intermediate_ca_csr(root_key_path, root_cert_path, intermediate_csr_path, intermediate_cert_path):
    ...
    intermediate_cert = x509.CertificateBuilder().subject_name(
        ...
        x509.SubjectKeyIdentifier.from_public_key(intermediate_csr.public_key()), critical=False,
...
def sign_end_entity_csr(intermediate_key_path, intermediate_cert_path, end_entity_csr_path, end_entity_cert_path):
    ...
    end_entity_cert = x509.CertificateBuilder().subject_name(
        ...
        x509.AuthorityKeyIdentifier.from_issuer_public_key(intermediate_private_key.public_key()), critical=False,
    ).add_extension(
        x509.SubjectKeyIdentifier.from_public_key(end_entity_csr.public_key()), critical=False,
    ...
    with open(end_entity_cert_path, "wb") as cert_file:
        cert_file.write(end_entity_cert.public_bytes(serialization.Encoding.PEM))
