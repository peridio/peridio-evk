...
from log import log_cli_command, log_cli_response, log_modify_file, log_error, log_info
...
def peridio_cli(command):
    ...
    try:
        ...
        stdout_thread = threading.Thread(target=stream_stdout, args=(process.stdout, stdout_queue))
        stderr_thread = threading.Thread(target=stream_stderr, args=(process.stderr, stderr_queue))
        stdout_thread.start()
        stderr_thread.start()
        ...
        result = SubprocessResult(str(stdout_output), str(stderr_output), return_code)
        return result
...
def filter_dicts(dict_list, key, value):
    return [dictionary for dictionary in dict_list if dictionary.get(key) == value]
